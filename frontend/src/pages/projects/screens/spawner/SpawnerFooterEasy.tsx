...
const SpawnerFooterEasy: React.FC<SpawnerFooterProps> = ({
  startNotebookData,
  storageData,
  envVariables,
  dataConnection,
  ...
  const onCreateNotebook = async () => {
    ...
    const apiKeyForNotebookResult = await createApiKeyForNotebook(
      username,
      projectName,
      translateDisplayNameForK8s(startNotebookData.notebookName),
      startNotebookData.image.imageStream?.metadata.name || '',
    ).catch(handleError);
    if (!apiKeyForNotebookResult) {
      return;
    }
    const apiKeyForNotebook: EnvVariable[] = apiKeyForNotebookResult;
      ...
      <StackItem>
        <ActionList>
          <ActionListItem>
            ...
            >
              {editNotebook ? 'Update' : 'Create'} application
            </Button>
          </ActionListItem>
          <ActionListItem>
            <Button
...
export default SpawnerFooterEasy;
